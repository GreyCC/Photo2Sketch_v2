...
# classes
class P2SMultiWeighting(nn.Module):
    def __init__(self):
        super(P2SMultiWeighting, self).__init__()
        ...
        self.reassemble = nn.Conv2d(5, 3, 1)
    ...
    def forward(self, x):
        ...
        out_1 = out ** 0.6 * x
        out_1 = out_1[:, 0:1, :, :] * 0.299 + out_1[:, 1:2, :, :] * 0.587 + out_1[:, 2:3, :, :] * 0.114

        out_2 = out ** 0.8 * x
        out_2 = out_2[:, 0:1, :, :] * 0.299 + out_2[:, 1:2, :, :] * 0.587 + out_2[:, 2:3, :, :] * 0.114

        out_3 = out * x
        out_3 = out_3[:, 0:1, :, :] * 0.299 + out_3[:, 1:2, :, :] * 0.587 + out_3[:, 2:3, :, :] * 0.114

        out_4 = out ** 1.2 * x
        out_4 = out_4[:, 0:1, :, :] * 0.299 + out_4[:, 1:2, :, :] * 0.587 + out_4[:, 2:3, :, :] * 0.114

        out_5 = out ** 1.4 * x
        out_5 = out_5[:, 0:1, :, :] * 0.299 + out_5[:, 1:2, :, :] * 0.587 + out_5[:, 2:3, :, :] * 0.114
        return out_1, out_2, out_3, out_4, out_5


class P2SUnet_multi_weight(nn.Module):
    def __init__(self):
        super(P2SUnet_multi_weight, self).__init__()
        self.slice4 = ResidualBlock(512, 256)
        self.slice3 = ResidualBlock(256, 128)
        self.slice2 = ResidualBlock(128, 64)
        self.slice1 = ResidualBlock(64, 64, upsample=False)
    ...
    def forward(self, x):
        feat, feat1, feat2, feat3 = self.E(x)
        h = self.slice4(feat) + feat3
        h = self.slice3(h) + feat2
        h = self.slice2(h) + feat1
        h = self.slice1(h)
        score = self.confidence(h)
        score = self.soft(score)
        out = self.map(h) * score
...
class EncoderUnet(nn.Module):
    ...
    def forward(self, HR):
        ...
        feat_multi_attn = F.interpolate(feat4, size=(32, 32), mode='bilinear')
        ...
        feat_multi_attn = F.interpolate(feat_multi_attn, size=(feat4.shape[2], feat4.shape[3]), mode='bilinear')
        feat = feat4 + feat_multi_attn

        return feat, feat1, feat2, feat3


class P2SUnet(nn.Module):
    def __init__(self):
        super(P2SUnet, self).__init__()
        self.slice4 = ResidualBlock(512, 256)
        self.slice3 = ResidualBlock(256, 128)
        self.slice2 = ResidualBlock(128, 64)
        self.slice1 = ResidualBlock(64, 64, upsample=False)
    ...
    def forward(self, x):
        ...
        #     out[i] -= torch.min(out_norm[i])
        #     out[i] /= torch.max(out_norm[i])

        return out

class Inverse_Unet(nn.Module):
    def __init__(self):
        super(Inverse_Unet, self).__init__()
...
class Inverse_v2(nn.Module):
    def __init__(self):
        super(Inverse_v2, self).__init__()
...
class Img_decoder_v4(nn.Module):
    def __init__(self):
        super(Img_decoder_v4, self).__init__()
